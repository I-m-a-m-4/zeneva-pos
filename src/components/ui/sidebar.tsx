...
const SIDEBAR_WIDTH_ICON = "3.5rem" // Slightly increased for larger icons
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "[[data-side=left][data-state=collapsed]]/sidebar_&]:cursor-e-resize [[data-side=right][data-state=collapsed]]/sidebar_&]:cursor-w-resize",
...
>(({ className, ...props }, ref) => (
  <ul
    ref={ref}
    data-sidebar="menu"
    className={cn("flex w-full min-w-0 flex-col gap-1 group-[[data-state=collapsed][data-collapsible=icon]]/sidebar:gap-2", className)}
    {...props}
  />
))
...
>(
  (
    ...
    // Combined active and general interaction states
    const combinedClasses = cn(
      sidebarMenuButtonVariants({ variant, size, className }),
      // General click (mouse down) state
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground", // Keep active sub-items consistent for now
        size === "sm" && "text-xs",
        size === "md" && "text-sm",
        "group-data-[collapsible=icon]/sidebar:hidden",
        className
